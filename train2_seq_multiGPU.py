...
print('line35', n_gpus)
def setup_process(rank, world_size, backend='nccl'): # nccl or gloo
    """ Initialize the distributed environment. """
    os.environ['MASTER_ADDR'] = 'kw61312' # localhost ?
    os.environ['MASTER_PORT'] = '29500' # which port?
    dist.init_process_group(backend = backend, init_method='env://',timeout=timedelta(seconds=20))
...
parser.add_argument('--id', type=str, default='train_all_scenarios', help='Unique experiment identifier.') # BEFORE IT WAS test
...
parser.add_argument('--batch_size', type=int, default=20, help='Batch size')	# default=24
...
class Engine(object):
	...
	def train(self):
		...
		for data in pbar:
			...
			num_batches += 1
			optimizer.step()
#			cleanup() # to destroy process group
...
root_csv_train = './Dataset/deepsense_challenge2023_trainset.csv'
dataset_scen = MATTIA_Data(root_csv=root_csv_train,config=config, test=False,augment={'camera':0, 'gps':0})

train_test_split = 0.9 # PUT THESE HYPERPARAMETERS INTO CONFIG() FILE OR args(), maybe better in args()
train_val_split = 0.8

train_dim = int(train_test_split * len(dataset_scen))
train_dim1 = int(train_val_split * train_dim)
train_set, test_set = torch.utils.data.random_split(dataset_scen,[train_dim, len(dataset_scen) - train_dim])
train_set, val_set = torch.utils.data.random_split(train_set,[train_dim1,len(train_set) - train_dim1])
train_train_set, val_set = torch.utils.data.random_split(dataset_scen,[train_dim,len(dataset_scen) - train_dim])


train_sampler = torch.utils.data.distributed.DistributedSampler(train_set)
dataloader_train = DataLoader(train_set,batch_size=args.batch_size,shuffle=False, num_workers=num_workers, pin_memory=False,
								  worker_init_fn=seed_worker, generator=g, sampler=train_sampler)
val_sampler = torch.utils.data.distributed.DistributedSampler(val_set)
dataloader_val = DataLoader(val_set,batch_size=args.batch_size,shuffle=False, num_workers=num_workers, pin_memory=False,
								  worker_init_fn=seed_worker, generator=g, sampler=val_sampler)
test_sampler = torch.utils.data.distributed.DistributedSampler(test_set)
dataloader_test = DataLoader(test_set,batch_size=args.batch_size,shuffle=False, num_workers=num_workers, pin_memory=False,
								  worker_init_fn=seed_worker, generator=g, sampler=test_sampler)
train_set, val_set = torch.utils.data.random_split(dataset_scen,[20,len(dataset_scen) - 20])
